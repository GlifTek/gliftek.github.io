 ...
 * Mon Nov 15 2021 07:06:29 GMT-0500 (Eastern Standard Time)
...
this.procedures["SET_STYLES"] = SET_STYLES;
...
this.procedures["MAKE_BUTTONS2"] = MAKE_BUTTONS2;
this.procedures["MAKE_BUTTONS"] = MAKE_BUTTONS;
this.procedures["BUTTON_RESET"] = BUTTON_RESET;
this.procedures["UPLOAD_SVG"] = UPLOAD_SVG;
this.procedures["SVG_LOADER"] = SVG_LOADER;
...
this.procedures["ROTATE_SEPARATE_OBJECTS"] = ROTATE_SEPARATE_OBJECTS;
...
var PROC = {
    ...
    "ROTATE_SEPARATE_OBJECTS": ROTATE_SEPARATE_OBJECTS,
    "OBJECT_ROTATION": OBJECT_ROTATION,
    "ROTATE_OBJECT": ROTATE_OBJECT,
};

var element_name, object_to_extrude, depth, file, svg_session_id, LIST_Meshes_From_Extrusion, input, object_to_rotate, m, Current_SVG, Current_SVG_Extruded_Master_Control, LIST_SVG_Parts_Extruded, LIST_Controls_From_Extrusion, SVG_Empty_Scale, Current_SVG_Scale;
...
function SET_STYLES(element_name) {
  setHTMLElemStyle('fontFamily', 'arial', element_name, false);
  setHTMLElemStyle('color', 'white', element_name, false);
  setHTMLElemStyle('fontSize', '40', element_name, false);
  setHTMLElemStyle('fontStyle', 'bold', element_name, false);
  setHTMLElemStyle('background', 'transparent', element_name, false);
  setHTMLElemStyle('border', '0', element_name, false);
}
...
//  CORE CODE
            ...
            let extrudeSessionID_ToEval =
            `window.LIST_${objInput.name}_Extruded_meshes = [];
            window.LIST_${objInput.name}_Extruded_controls = [];`;
            console.log('Extrude Geometry - extrudeSessionID_ToEval:',extrudeSessionID_ToEval);
            eval(extrudeSessionID_ToEval);

            let master_Empty = new v3d.Object3D;
            // master_Empty.DefaultUp = new THREE.Vector3(0,0,1);
            // master_Empty.rotateX( (Math.PI / 2) * -1 );

            master_Empty.name = 'SVG_' + objInput.name + '_Extruded' + '_Master_Control';
            app.scene.add(master_Empty);

            if ( axesHelpersCheckbox == true )
            {
                const master_axesHelper = new THREE.AxesHelper( 2 );
                master_axesHelper.DefaultUp = new THREE.Vector3(0,0,1);
                master_axesHelper.rotateX( Math.PI / -2);
                master_Empty.attach(master_axesHelper);
            };
            ...
            function extrudeObject(obj)
            {
                ...
                svgDataPaths.forEach((path) =>
                {
                    ...
                    console.log('Extrude Geometery - path.id: ',path.id);
                    ...
                    // else if ( path.id == 'undefined' )
                    else if ( typeof path.id == 'undefined' )
                    {
                        path_ID = objInput.name; // plus iteration number?!??!
                        doShapes();
                    };
                    ...
                    function doShapes()
                    {
                        ...
                        shapes.forEach((shape) =>
                        {
                            ...
                            if ( separateMaterialsCheckbox == true )
                            {
                                ...
                                material_frontAndBack.name = 'SVG_' + path_ID + '_frontAndBack';
                                ...
                                material_Sides.name = 'SVG_' + path_ID + '_Sides';
                            ...
                            else if ( separateMaterialsCheckbox == false )
                            {
                                ...
                                material.name = 'SVG_' + path_ID + '_Extruded';
                            ...
                            else if ( svgDataPaths.length = 1 )
                            {
                                ...
                                // mesh.name = nameInput;
                                mesh.name = 'SVG_' + path_ID + '_Extruded_Mesh';
                                ...
                                let single_Empty = new v3d.Object3D;
                                single_Empty.name = 'SVG_' + path_ID + '_Extruded' + '_Control';
                                app.scene.add(single_Empty);
                                // svgGroup.add(single_Empty);
                                //app.scene.add(svgGroup);


                                // let add_items_to_extrudeSessionID_ToEval =
                                // `window.LIST_${objInput.name}_Extruded_meshes.push(mesh.name);
                                // window.LIST_${objInput.name}_Extruded_controls.push(single_Empty.name);
                                // console.log('Extrude Geometry - window.LIST_${objInput.name}_Extruded_meshes:',window.LIST_${objInput.name}_Extruded_meshes);
                                // console.log('Extrude Geometry - window.LIST_${objInput.name}_Extruded_controls:',window.LIST_${objInput.name}_Extruded_controls);
                                // `;
                                ...
                                // console.log('window.LIST_${objInput.name}_Extruded_meshes:',add_items_to_extrudeSessionID_ToEval);
                                // console.log('window.LIST_${objInput.name}_Extruded_controls:',add_items_to_extrudeSessionID_ToEval);
                                eval(add_items_to_extrudeSessionID_ToEval);


                                console.log('Extrude Geometry - svgGroup:',svgGroup);


                                if ( axesHelpersCheckbox == true )
                                {
                                    let axesHelper = new THREE.AxesHelper( 1 );
                                    axesHelper.DefaultUp = new THREE.Vector3(0,0,1);
                                    axesHelper.rotateX( Math.PI / -2);
                                    single_Empty.attach(axesHelper);
                                }


                                single_Empty.position.x = getCenterPoint(mesh).x * ( scaleLevel * -1 );
                                single_Empty.position.y = getCenterPoint(mesh).y * scaleLevel;
                                single_Empty.position.z = getCenterPoint(mesh).z * scaleLevel;

                                single_Empty.attach(mesh);

                                single_Empty.translateX( obj.svgSize.x * ( scaleLevel * -1 ));
                                single_Empty.translateY( obj.svgSize.y * scaleLevel );

                                single_Empty.translateZ( (depthScaleInput / 100) / 4 );

                                master_Empty.attach(single_Empty);
                ...
                // which makes centering them in the group easy
                    ...
                    // REVIEW THIS IT ISNT BEING USED.. WAS FOR GROUPS???
                function offsetObjectsToCenter()
                {
                    ...
                    // svgGroup.name = objInput.name + '_Extruded';
...
function BUTTON_UPLOAD() {
  ...
  setHTMLElemAttribute('innerHTML', 'OR CLICK HERE TO UPLOAD LOCAL FILE', 'button_upload', false);
  SET_STYLES('button_upload');
  BUTTONS_POINTER('button_upload');
  eventHTMLElem('pointerup', 'button_upload', false, function(event) {
    openFile(function() {
      UPLOAD_SVG(_pGlob.openedFile, 'local_uploaded_file', 100);
    });
  });
}
...
// Describe this function...
function FORM_URL() {
  addHTMLElement('a', 'form_url_text', 'TO', ['CONTAINER'], false);
  setHTMLElemAttribute('innerHTML', 'LOAD FROM URL: ', 'form_url_text', false);
  SET_STYLES('form_url_text');
  addHTMLElement('input', 'form_url', 'TO', ['CONTAINER'], false);
  setHTMLElemAttribute('value', 'https://www.gliftek.com/verge_3d_plugins/assets/demos/GLIFTEK_Plugins_SVG_Loader/Recycling_symbol.svg', 'form_url', false);
  SET_STYLES('form_url');
  setHTMLElemStyle('borderWidth', '1', 'form_url', false);
  eventHTMLElem('change', 'form_url', false, function(event) {
    UPLOAD_SVG(getHTMLElemAttribute('value', 'form_url', false), 'url_file', 200);
  });
}
...
// Describe this function...
function UPLOAD_SVG(file, svg_session_id, depth) {
  SVG_LOADER(file, svg_session_id, depth);
  registerSetTimeout(1, function() {
    assignMat(LIST_Meshes_From_Extrusion, 'cube_material');

        copyMaterialFunction
            (
                'cube_material',
                'SVG_uploadedFile_Extruded'
            );
        ROTATE_OBJECT(Current_SVG_Extruded_Master_Control);
  });
}
...
//  DECLARE VARIABLES
    ...
    let SVGname,SVGheightHalf,SVGwidthHalf,SVGheightHalf_Neg2,SVGwidthHalf_Neg2;
...
//  APPLY SVG LOADER
                ...
                console.log('svgSessionIDInput:',svgSessionIDInput);
                ...
                function fileInputCheck()
                {
                    ...
                    let svgFileInput_Length = svgFileInput.length;

                    let first_Four_Letters = svgFileInput.substring(0, 4);
                    console.log('SVG Object Loader - first_Four_Letters: ',first_Four_Letters);

                    if ( first_Four_Letters == 'http')
                    {
                        // var filename = svgFileInput.replace(/^.*[\\\/]/, '');
                        let last_Backslash = svgFileInput.lastIndexOf("/") + 1;
                        let filenameFull = svgFileInput.substr(last_Backslash);
                        console.log('SVG Object Loader - filenameFull: ',filenameFull);

                        let last_Period = filenameFull.lastIndexOf(".") + 1;
                        let filename_Extension = filenameFull.substr(last_Period);
                        console.log('SVG Object Loader - filename_Extension: ',filename_Extension);

                        if ( filename_Extension == 'svg' || filename_Extension == 'SVG' ||
                        filename_Extension == 'svg ' || filename_Extension == 'SVG ')
                        {

                            let filenameFull_Length = filenameFull.length;

                            let SVG_Filename = filenameFull.substring(0, filenameFull_Length-4);
                            console.log('SVG Object Loader - SVG_Filename: ',SVG_Filename);
                    ...
                    else if ( first_Four_Letters == 'data')
                    {
                        SVG_Filename = 'uploadedFile'
                ...
                function loadSVG( url )
                {
                    ...
                    // let svg_SessionID_ToEval =
                    // `window.LIST_${svgSessionIDInput} = filename;
                    // console.log('SVG Object Loader - window.LIST_${svgSessionIDInput} = svgSessionIDInput:',window.LIST_${svgSessionIDInput} = svgSessionIDInput);
                    // `;
                    // console.log('SVG Object Loader - svg_SessionID_ToEval:',svg_SessionID_ToEval);
                    // eval(svg_SessionID_ToEval);


                    // THREE.SVGLoader.crossOrigin = '';
                    // THREE.SVGLoader.crossOrigin = 'anonymous';
                    ...
                    loader.load( url, function ( data )
                    {
                        ...
                        else
                        {
                            // if ( window.)
                            // let SVG_number = '1';
                            // SVGname = 'SVG_' + SVG_number;

                            SVGname = SVG_Filename;
                        ...
                        function setWindow_SVG_SessionID()
                        {
                            ...
                            let svg_SessionID_ToEval =
                            `window.${svgSessionIDInput} = SVGname;
                            console.log('SVG Object Loader - window.${svgSessionIDInput}: ',window.${svgSessionIDInput});
                            `;
                            console.log('SVG Object Loader - svg_SessionID_ToEval:',svg_SessionID_ToEval);
                            eval(svg_SessionID_ToEval);
                        ...
                        let SVGheight = data.xml.attributes.height.value;
                        console.log('SVGheight: ',SVGheight);

                        let SVGwidth = data.xml.attributes.width.value;
                        ...
                        {
                            // console.log('SVG Object Loader - SVG height/width item: ',item);

                            let len = item.length;
                            if ( item.substring(len-2, len-0) == 'px')
                            {
                                let string = item.substring(0, len-3);
                        ...
                        SVGheight = removePX(SVGheight);
                        // console.log('SVGheight AFTER removing PX: ',SVGheight);

                        SVGwidth = removePX(SVGwidth);
                        // console.log('SVGwidth AFTER removing PX: ',SVGwidth);


                        SVGheightHalf =  SVGheight / 2 ;
                        console.log('SVGheightHalf: ',SVGheightHalf);

                        SVGwidthHalf = SVGwidth  / 2 ;
                        console.log('SVGwidthHalf: ',SVGwidthHalf);


                        SVGheightHalf_Neg2 =  SVGheight / - 2 ;
                        console.log('SVGheightHalf: ',SVGheightHalf);

                        SVGwidthHalf_Neg2 = SVGwidth  / - 2 ;
                        console.log('SVGwidthHalf: ',SVGwidthHalf);
                        ...
                        function processNode( node, currentGroup )
                        {
                            ...
                            if ( node.nodeName === 'g' )
                            {
                                ...
                                for ( let i = 0; i < paths.length; i ++ )
                                {
                                    ...
                                    if ( path.userData.node === node ) {
                                        ...
                                        createPathGeometry( path, currentGroup );
                                        break;
                            ...
                            for ( let i = 0; i < nodes.length; i ++ ) {
                                ...
                                processNode( nodes[ i ], currentGroup);
                        ...
                        function createPathGeometry( path, currentGroup )
                        {
                            ...
                            for ( let j = 0; j < shapes.length; j ++ )
                            {
                                ...
                                mesh.position.x = SVGwidthHalf_Neg2;
                                mesh.position.y = SVGheightHalf_Neg2;
...
//  DECLARE VARIABLES
        ...
        const puzzleName = 'Get SVG Object';
        let result;
...
function getItemsFromExtrusion() {
    return (function
...
//  DECLARE VARIABLES
        ...
        const puzzleName = 'Get Items From Extrusion';
        let result;
...
//  CORE CODE
        ...
        const toEval = `result = window.LIST_${objInput.name}_Extruded_${typeDropdown}`;

        eval(toEval);

        console.log('Extrude Geometry - window.LIST_${objInput.name}_Extruded_Controls');
...
// Describe this function...
function SVG_LOADER(file, svg_session_id, depth) {
  RESET_OBJECTS();
    ...
    Current_SVG =
        ...
        ;
    Current_SVG_Extruded_Master_Control = ['SVG_',Current_SVG,'_Extruded_Master_Control'].join('');
    LIST_Controls_From_Extrusion =
        getItemsFromExtrusion
        (
            Current_SVG,
            'controls',
        )
        ;
    LIST_Meshes_From_Extrusion =
        getItemsFromExtrusion
        (
            Current_SVG,
            'meshes',
        ...
        ;
    console.log('LIST_Meshes_From_Extrusion: ');
    console.log(LIST_Meshes_From_Extrusion);
    EXTRUDE_OBJECTS(Current_SVG, depth);
  });
}
...
function RESET_OBJECTS() {
  ...
  removeObject(Current_SVG_Extruded_Master_Control);
  removeObject(LIST_Meshes_From_Extrusion);
  removeObject(LIST_Controls_From_Extrusion);
...
function SETUP() {
  ...
  changeVis('Plane', false);
      ...
      createObject('PLANE', 'Plane', 10, 10, 0, 0, 0, 0, 'PERSPECTIVE', 'AMBIENT', 0);
  setObjTransform('Plane', false, 'rotation', [90, '', ''], false);
  setObjTransform('Plane', false, 'position', ['', '', -0.01], false);
  setMaterialColor('PlaneMaterial', 'color', 0, 0, 0, '#2F4F4F');
  createObject('LIGHT', 'Ambient Light', 0, 0, 0, 0, 0, 0, 'PERSPECTIVE', 'AMBIENT', 0);
}

// Describe this function...
function VERGE_3D_LOGO(file, svg_session_id, depth) {
  SVG_LOADER(file, svg_session_id, depth);
  registerSetTimeout(0.5, function() {
    registerSetTimeout(1, function() {
      setObjTransform(Current_SVG, false, 'position', [-2, '', 1.25], false);
      setObjTransform(Current_SVG_Extruded_Master_Control, false, 'position', ['', 0.5, ''], false);
      SVG_Empty_Scale = 0.5;
      setObjTransform(Current_SVG_Extruded_Master_Control, false, 'scale', [SVG_Empty_Scale, SVG_Empty_Scale, SVG_Empty_Scale], false);
      setObjTransform(Current_SVG_Extruded_Master_Control, false, 'position', [2, 0, 1.25], false);
      Current_SVG_Scale = 0.0025;
      setObjTransform(Current_SVG, false, 'scale', [Current_SVG_Scale, Current_SVG_Scale, -Current_SVG_Scale], false);
      setObjTransform('SVG_fill_Extruded_Control', false, 'scale', ['', 0.35, ''], false);
      assignMat('SVG_fill_Extruded_Mesh', 'cube_material');
...
// Describe this function...
function BUTTONS_POINTER(element_name) {
  eventHTMLElem('mouseover', element_name, false, function(event) {
    setHTMLElemStyle('cursor', 'pointer', ['CONTAINER'], false);
  });
  eventHTMLElem('pointerout', element_name, false, function(event) {
    setHTMLElemStyle('cursor', 'default', ['CONTAINER'], false);
  });
}

// Describe this function...
function GLIFTEK_LOGO(file, svg_session_id, depth) {
  SVG_LOADER(file, svg_session_id, depth);
  registerSetTimeout(0.5, function() {
    setObjTransform(Current_SVG, false, 'position', ['', '', 1.5], false);
    setObjTransform(Current_SVG, false, 'scale', [0.0025, 0.0025, -0.0025], false);
    registerSetTimeout(1, function() {
      setObjTransform(Current_SVG_Extruded_Master_Control, false, 'position', ['', '', 0.5], false);
      changeVis('Plane', true);
      ROTATE_SEPARATE_OBJECTS(LIST_Controls_From_Extrusion);
    });
  });
}
...
// Describe this function...
function ROTATE_SEPARATE_OBJECTS(input) {
...
function OBJECT_ROTATION() {
  ROTATE_OBJECT(Current_SVG_Extruded_Master_Control);
  ROTATE_SEPARATE_OBJECTS(LIST_SVG_Parts_Extruded);
}
...
function ROTATE_OBJECT(object_to_rotate) {
  ...
  animateParam(0, 359, 10, 'Linear', 'InOut', Infinity, false,
      function() {
...
'SVG_Recycling_symbol';

'SVG_Recycling_symbol';

'<none>';

getObjectMaterial('<none>');
...
'SVG_Bluetooth_Logo_Extruded_Mesh';
