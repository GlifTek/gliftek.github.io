 ...
 * Sun Aug 22 2021 14:31:24 GMT-0400 (Eastern Daylight Time)
...
this.procedures["MAKE_CLONES"] = MAKE_CLONES;
this.procedures["MAIN"] = MAIN;
this.procedures["SET_BUTTONS"] = SET_BUTTONS;
this.procedures["MAKE_DROPDOWNS"] = MAKE_DROPDOWNS;
this.procedures["CONSTRAINTS"] = CONSTRAINTS;
this.procedures["CONSTRAINTS_NEW"] = CONSTRAINTS_NEW;
this.procedures["RESET_SCREEN"] = RESET_SCREEN;

var PROC = {
    "MAKE_CLONES": MAKE_CLONES,
    "MAIN": MAIN,
    "SET_BUTTONS": SET_BUTTONS,
    "MAKE_DROPDOWNS": MAKE_DROPDOWNS,
    "CONSTRAINTS": CONSTRAINTS,
    "CONSTRAINTS_NEW": CONSTRAINTS_NEW,
    "RESET_SCREEN": RESET_SCREEN,
};

var Cube_Blue_New, Cube_Red_New, dropdown_value, buttons, button_1_InnerHTML;
...
function MAKE_CLONES() {
  removeObject('myNewObject');
  changeVis('Cube_Blue', true);
  changeVis('Cube_Red', true);
  Cube_Blue_New = cloneObject('Cube_Blue');
  Cube_Red_New = cloneObject('Cube_Red');
  changeVis('Cube_Blue', false);
  changeVis('Cube_Red', false);
  CONSTRAINTS();
}
...
// ALL ARGUMENTS USED FROM INPUTS
    ...
    (
        ...
        objInput1,
        objInput2,
        resultObjectNameInput
    )
    {   //  BEGIN Plug.provide Function
//________________________________________________________________________________
//  DECLARE VARIABLES

    const puzzleName = 'CSG Boolean';
...
//  PUZZLE CORE CODE
        ...
        function checkType()
        {
            ...
            {
                booleanType = typeDropdown;
                jsFileLoaderTop(wait);
            }

            else
            {
                booleanType = typeInput;
                jsFileLoaderTop(wait);
            }
        }
        ...
        function CSG_Function()
        {

            let meshA = app.scene.getObjectByName(objInput1);
            console.log('objInput1: ',objInput1);
            let meshB = app.scene.getObjectByName(objInput2);
            console.log('objInput2: ',objInput2);
            ...
            // let bspResult_preEval = `bspA.${typeDropdown}(bspB);`
            let bspResult_preEval = `bspA.${booleanType}(bspB);`
            let bspResult = eval(bspResult_preEval);
            ...
            meshResult.name = resultObjectNameInput;
        ...
        // jsFileLoaderTop(CSG_Function);

        function jsFileLoaderTop(callbackFunction)
            {
                ...
                function jsFileLoader()   {   // Loads all Js dependencies in order.

                    let i = 0;
                    includeJs ();

                        function includeJs () {
                            let jsFilesCallback = function ()
                            {   if (i >= (jsFiles.length - 1))
                                { callbackFunction() }
                                else if (i < (jsFiles.length - 1))
                                { ++i; includeJs() };   }

                            let js = document.createElement("script");
                            js.type = "text/javascript";
                            js.src = jsFilePath + jsFiles[i];
                            js.onload = jsFilesCallback;
...
function MAIN() {
  ...
  SET_BUTTONS();
      ...
      registerOnDrag(Cube_Blue_New, false, [0,1,2], function() {}, function() {
    dragMove(Cube_Blue_New, 'XY', '@3T}C9@?,s`c)%/Rz#77', 'N8RJ6OAW=bZ5s.c:3YYT');
  }, function() {}, 'N8RJ6OAW=bZ5s.c:3YYT');
  registerOnDrag(Cube_Red_New, false, [0,1,2], function() {}, function() {
    dragMove(Cube_Red_New, 'XY', '/GNa(0db;vxM]|OH!f_]', '?e3{UQ/QbgynyM]!1^g(');
  }, function() {}, '?e3{UQ/QbgynyM]!1^g(');
  registerOnClick(Cube_Blue_New, false, true, [0,1,2], function() {
    dropdown_value = getHTMLElemAttribute('value', 'dropdown_boolean_type', false);
    console.log('dropdown_boolean_type: ' + String(dropdown_value));
      ...
      removeObject(Cube_Blue_New);
      removeObject(Cube_Red_New);
      registerOnDrag('myNewObject', false, [0,1,2], function() {}, function() {
        dragMove('myNewObject', 'XY', 'x?bv}X@2$~NyIT9Itx8%', 'Y*j*v]b6%9auOWf+pjGZ');
        CONSTRAINTS_NEW();
      }, function() {}, 'Y*j*v]b6%9auOWf+pjGZ');
    });
  }, function() {});
  registerOnClick(Cube_Red_New, false, true, [0,1,2], function() {
    dropdown_value = getHTMLElemAttribute('value', 'dropdown_boolean_type', false);
    console.log('dropdown_boolean_type: ' + String(dropdown_value));
      ...
      removeObject(Cube_Red_New);
      registerOnDrag('myNewObject', false, [0,1,2], function() {}, function() {
        dragMove('myNewObject', 'XY', 'E/8{Qru7bDwY)+X3Bf^?', 'b)^jBxVc1o.%oTB+cZvl');
        CONSTRAINTS_NEW();
      }, function() {}, 'b)^jBxVc1o.%oTB+cZvl');
    });
  }, function() {});
}
...
function SET_BUTTONS() {
  addHTMLElement('div', 'div_controls', 'TO', ['CONTAINER'], true);
  setHTMLElemStyle('zIndex', '5', 'div_controls', false);
  setHTMLElemAttribute('width', '100%', 'div_controls', false);
  addHTMLElement('button', 'button_reset', 'TO', 'div_controls', false);
  setHTMLElemAttribute('width', '200px', 'button_reset', false);
  addHTMLElement('select', 'dropdown_boolean_type', 'TO', 'div_controls', false);
  setHTMLElemAttribute('width', '200px', 'dropdown_boolean_type', false);
  MAKE_DROPDOWNS();
  buttons = ['dropdown_boolean_type', 'button_reset'];
  ...
  'reset scene' + '\n' +
  '</div>');
  setHTMLElemAttribute('innerHTML', button_1_InnerHTML, 'button_reset', false);
  eventHTMLElem('pointerover', buttons, false, function(event) {
    setHTMLElemStyle('cursor', 'pointer', buttons, false);
  });
  eventHTMLElem('click', 'button_reset', false, function(event) {
    console.log('button_reset CLICKED!');
    RESET_SCREEN();
  });
}
...
function MAKE_DROPDOWNS() {
  addHTMLElement('option', 'option1', 'TO', 'dropdown_boolean_type', false);
  setHTMLElemAttribute('value', 'subtract', 'option1', false);
  setHTMLElemAttribute('innerHTML', 'subtract', 'option1', false);
  /* _______________________________ */
  addHTMLElement('option', 'option2', 'TO', 'dropdown_boolean_type', false);
  setHTMLElemAttribute('value', 'union', 'option2', false);
  setHTMLElemAttribute('innerHTML', 'union', 'option2', false);
  /* _______________________________ */
  addHTMLElement('option', 'option3', 'TO', 'dropdown_boolean_type', false);
  setHTMLElemAttribute('value', 'intersect', 'option3', false);
  setHTMLElemAttribute('innerHTML', 'intersect', 'option3', false);
}
...
function CONSTRAINTS() {
  objConstraintAddLimit('myConst1_Blue2', 'Cube_Blue2', 'POS_X', -4, 4, undefined, undefined, undefined);
  objConstraintAddLimit('myConst2_Blue2', 'Cube_Blue2', 'POS_Y', -4, 4, undefined, undefined, undefined);
  objConstraintAddLimit('myConst1_Red2', 'Cube_Red2', 'POS_X', -4, 4, undefined, undefined, undefined);
  objConstraintAddLimit('myConst2_Red2', 'Cube_Red2', 'POS_Y', -4, 4, undefined, undefined, undefined);
}

// Describe this function...
function CONSTRAINTS_NEW() {
  objConstraintAddLimit('myConst1_New', 'myNewObject', 'POS_X', -4, 4, undefined, undefined, undefined);
  objConstraintAddLimit('myConst2_New', 'myNewObject', 'POS_Y', -4, 4, undefined, undefined, undefined);
}
...
function RESET_SCREEN() {
  if (isObjectVisible('myNewObject')) {
    MAKE_CLONES();
  }
}
