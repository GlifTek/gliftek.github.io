 ...
 * Fri Aug 20 2021 12:15:43 GMT-0400 (Eastern Daylight Time)
...
this.procedures["GENERATE_SKY_AND_OCEAN"] = GENERATE_SKY_AND_OCEAN;
...
var PROC = {
    "GENERATE_SKY_AND_OCEAN": GENERATE_SKY_AND_OCEAN,
    "MAIN": MAIN,
...
// ALL ARGUMENTS USED FROM INPUTS
    ...
    (
        ...
        // skySun_spotlight vars
        // spotlightTarget,
        // skySunDistance,
        // skySunPenumbra,
        // skySunPower,

        // skySunCastShadowCheckbox,
        // skySunShadowMapWidth,
        // skySunShadowMapHeight,
...
//  MAIN FUNCTION
        ...
        let sundisk1, sundisk2;

        // const sunAngularDiameterCosVar = 0.99995;   // normal
        // const sunAngularDiameterCosVar = 0.99999;   // smallest


        // let sundiskSize = 1.02039;
        ...
        const sunAngularDiameterCosVarSized = NewValue;

        console.log('sundiskSize:',sundiskSize);
        console.log('sunAngularDiameterCosVarSized:',sunAngularDiameterCosVarSized);

        window.skySunVector = null;
        window.skySunVectorScaled = null;
...
function sundisk()
{
    if (sundiskCheckbox == true)
    {
        sundisk1 = `'	float sundisk = smoothstep( ${sunAngularDiameterCosVarSized}, ${sunAngularDiameterCosVarSized} + 0.00002, cosTheta );'`;

        sundisk2 = `'	L0 += ( vSunE * 19000.0 * Fex ) * sundisk;'`;
    }
    else if (sundiskCheckbox == false)
    {
        sundisk1 = null;
...
function fragShade ()
{
    ...
    const fragVarJoined = eval(fragVar).join('\r\n');

    return fragVarJoined;
  };
...
//  PUZZLE CORE CODE
        ...
        //skySun variables for advanced mode or separate 'sun' puzzle?
        // let spotlightTarget,
        // skySunDistance,
        // skySunPenumbra,
        // skySunPower,
        ...
        function makeSky()
        {
            ...
            sky = new Sky();
            window.skySizeWin = skySize;
            sky.scale.setScalar( skySize );
            // sky.scale.setScalar( skySizeWin );
            // sky.scale.setScalar( 450000 );

            window.skyWin = sky;
            skyWin.name = 'Sky';
            console.log('skyWin: ',skyWin);
            ...
            var effectController = {
                ...
                const phi = THREE.MathUtils.degToRad( 90 - effectController.inclination );
                const theta = THREE.MathUtils.degToRad( effectController.azimuth );
                let sunRadius = 1;

                skySunVector = new THREE.Vector3();
                skySunVector.setFromSphericalCoords( sunRadius, phi, theta );


                skySunVector.name = 'skySunVector';
                console.log('skySunVector: ',skySunVector);



                scaleUpSkySunVector();
                function scaleUpSkySunVector()
                {


                    let skySizeHalf = (skySize / 2);
                    console.log('skySizeHalf : ',skySizeHalf);
                    let skySizeFraction = (  skySizeHalf * .001 );
                    console.log('skySizeFraction : ',skySizeFraction);

                    let skySizeLesser = ( skySizeHalf - skySizeFraction );
                    console.log('skySizeLesser : ',skySizeLesser);



                    // let skySunVectorScaledUp = skySunVector.multiplyScalar( skySizeLesser );
                    let skySunVectorScaledUp = skySunVector.clone().multiplyScalar( skySizeLesser );
                    console.log('skySunVectorScaledUp: ',skySunVectorScaledUp);

                    let skySunVectorScaledUpX = skySunVectorScaledUp.x.toString().substr(0,10);
                    let skySunVectorScaledUpY = skySunVectorScaledUp.y.toString().substr(0,10);
                    let skySunVectorScaledUpZ = skySunVectorScaledUp.z.toString().substr(0,10);

                    window.skySunLocation = new THREE.Vector3(
                        skySunVectorScaledUpX,
                        skySunVectorScaledUpY,
                        skySunVectorScaledUpZ
                    );

                    // let skySunVectorScaledUp = skySunVector.multiplyScalar( skySizeWin );
                    // console.log('skySunVectorScaledUp: ',skySunVectorScaledUp);
                ...
                // let skySunVectorScalared = new THREE.Vector3(skySunVector.x,skySunVector.y,skySunVector.z);
                // console.log('skySunVectorScalared before scalaring : ',skySunVectorScalared);

                // let skySizeHalf = (skySize / 100);
                // console.log('skySizeHalf : ',skySizeHalf);
                // let skySizeLess = (  skySizeHalf * .001 );
                // console.log('skySizeLess : ',skySizeLess);

                // let skySizeLesser = ( skySizeHalf - skySizeLess );
                // console.log('skySizeLesser : ',skySizeLesser);
                ...
                // skySunVectorScalared.set(
                //     Math.round( (skySunVectorScalared.x * skySizeLesser),3 ),
                //     Math.round( (skySunVectorScalared.y * skySizeLesser),3 ),
                //     Math.round( (skySunVectorScalared.z * skySizeLesser),3 ),
                // );


                // console.log('skySunVectorScalared after scalaring : ',skySunVectorScalared);

                // skySunVectorScaled = skySunVectorScalared;

                // console.log('window.skySunVectorScaled : ',skySunVectorScaled);
                ...
                // // TODO: make light color dynamically follow changing sun shader circle color
                // let sunColor = '';
                // // let skySun_spotlight = new THREE.SpotLight( sunColor, sunIntensity );
                // let skySun_spotlight = new THREE.SpotLight( 0xffffff );
                // skySun_spotlight.name = 'skySun_spotlight';
                // skySun_spotlight.position.set (skySunVector);

                // // if object is selected, else default 0,0,0,
                // skySun_spotlight.target = skySun_spotlightTarget;

                // // skySun_spotlight.decay = 2; // realistic fallof from docs
                // skySun_spotlight.distance = skySunDistance;   //  default is 0.0
                // skySun_spotlight.decay = skySunDecay;   //  default is 1
                // skySun_spotlight.penumbra = skySunPenumbra; // faded zone around light cone (0-1) default 0.0.
                // skySun_spotlight.power = skySunPower; // default is 4Math.PI power = intensity * pi
                // skySun_spotlight.castShadow = skySunCastShadowCheckbox;
                // skySun_spotlight.shadow.mapSize.width = skySunShadowMapWidth;
                // skySun_spotlight.shadow.mapSize.height = skySunShadowMapHeight;

                // skySun_spotlight.shadow.camera.near = skySunShadowCamNear;
                // skySun_spotlight.shadow.camera.far = skySunShadowCamFar;
                // skySun_spotlight.shadow.camera.fov = skySunShadowCamFOV;

                // // skySun_spotlight.shadow.camera.near = 500;
                // // skySun_spotlight.shadow.camera.far = 4000;
                // // skySun_spotlight.shadow.camera.fov = 30;

                // skySun.add(skySun_spotlight);


                // let skySun_ambientlight = new THREE.ambientlight;
                // skySun_ambientlight.name = 'skySun_ambientlight';
                // skySun.add(skySun_ambientlight);


                // skySun.position = skySunVector;

                // app.scene.add(skySun);
                // console.log('skySun: ',skySun);
				...
				renderer.toneMappingExposure = effectController.exposure;
                    ...
                    uniforms[ 'sunPosition' ].value.copy( skySunVector );

                    console.log('uniforms sunPosition.value: ',uniforms[ "sunPosition" ].value);
...
//  DECLARE VARIABLES
		...
		const puzzleName = 'Generate Sun';

//________________________________________________________________________________
//  NO INPUT ERROR CHECK

// if ( !objInput || objInput == '<none>' ) {
//             console.log('[ GLIFTEK ]: ',puzzleName, 'puzzle needs an input!');
//     }   else    { mainFunction () };
...
//  PUZZLE CORE CODE
            ...
            // function skySunOrNewSun()
            // {
            //     console.log('useSkySunCheckbox : ',useSkySunCheckbox);
            //     // IF TRUE RUN PROMISE FUNCTION THEN USE SKYSUNVECTOR ELSE USE SETTINGS
            //     if ( useSkySunCheckbox == true )
            //     {
            //         // skySunPromiseWait();
            //         promiseFunction('skySunLocation', makeSun);
            //         // promisedItemFunction('skySunLocation', makeSun, false );
            //     }
            //     else if ( useSkySunCheckbox == false )
            //     {
            //         sunLocationFromSettings();
            //     };
...
//  PROMISE FUNCTION
                ...
                // // function  promiseFunction(promisedItem, successCallback)
				// {
                ...
                //             if (promisedItem.toString())
                //                 {
                //                     resolve(promisedItem.toString());
                //                 }
                ...
                //             console.log('promisedItem FOUND: ',promisedItem);
...
//________________________________________________________________________________
                ...
                function skySunPromiseWait()
                {

                    var timeout = 10000; // 1000000ms = 10 seconds

                    function ensure_skySunLocationIsSet(timeout) {
                        var start = Date.now();
                        return new Promise(waitFor_skySunLocation);

                        function waitFor_skySunLocation(resolve, reject) {
                            console.log("[ GLIFTEK Plugins ]:\n 'The Generate Sun' puzzle waiting for 'Generate Sky' puzzle sun.");

                            if (skySunLocation)
                            {
                                resolve(skySunLocation);
                            }
                            else if (timeout && (Date.now() - start) >= timeout)
                            {
                                reject(new Error("timeout. skySunLocation not found!"));
                                // );
                                // reject();
                                    console.log("[ GLIFTEK Plugins ]:\n 'The Generate Sun' puzzle needs a 'Generate Sky' puzzle\n to use it's sun for positioning.\n  ( un-check the checkbox if unwanted.)");
                                    alert("[ GLIFTEK Plugins ]:\n 'The Generate Sun' puzzle needs a 'Generate Sky' puzzle\n to use it's sun for positioning.\n  ( un-check the checkbox if unwanted.)");
                                // );
                            }
                            // GLIFTEK_WORLD_Generate_Sun_NEW_WORKING_ALMOST.block
                            else{
                                setTimeout(waitFor_skySunLocation.bind(this, resolve, reject), 30);
                            }
                        }
                    }

                    ensure_skySunLocationIsSet(timeout).then(function(){

                        console.log("skySunLocation found.")

                        // makeSun(withSkySun = true);skySunDirLight.position.set(skySunLocation);
                        makeSun(skySunLocation);

                    });
                };  //  END skySunPromiseWait



                function sunLocationFromSettings()
                {
                    const phi = THREE.MathUtils.degToRad( 90 - skySunInclination );
                    const theta = THREE.MathUtils.degToRad( skySunAzimuth );
                    let sunRadius = 1;

                    let sunVector = new THREE.Vector3();
                    sunVector.setFromSphericalCoords( sunRadius, phi, theta );


                    sunVector.name = 'sunVector';
                    console.log('sunVector: ',sunVector);
            ...
            function makeSun(sunLocation)
            {

                let skySunTargetObj;

                let sunColor = 'white';  // default is 0xffffff (white)#ffff00
                let sunIntensity = .05; // default is 1


                const skySunDirLight = new THREE.DirectionalLight( '#ffff00', 1 );
                skySunDirLight.color.setHSL( 0.1, 1, 0.95 );

                console.log('skySunCastShadowCheckbox : ',skySunCastShadowCheckbox);
                // skySunDirLight.castShadow = skySunCastShadowCheckbox;

                // skySunDirLight.position.set(sunLocation);
                skySunDirLight.position.set(
                    sunLocation.x,
                    sunLocation.y,
                    sunLocation.z,
                );

                console.log('skySunDirLight.position : ',skySunDirLight.position);

                skySunDirLight.name = 'skySunDirLight'
				app.scene.add( skySunDirLight );
                console.log('skySunDirLight : ',skySunDirLight);

                let sceneSun = app.scene.getObjectByName('skySunDirLight');
                console.log('sceneSun : ',sceneSun);
...
//  MAIN FUNCTION
        ...
        function mainFunction()
        {
            console.log('beginning ocean mainFunction');
            let  water, sun;
            // let water, waterSun, mesh;
            // let skySource = skyWin;
            // sunSource = skySun;
...
window.mirrorCamRendering = null;
...
scope.onBeforeRender = function ( renderer, scene, camera ) {
    ...
    // flag for lens flares to switch off/on visibility to avoid reflections
    // on to-do list to fix
    mirrorCamRendering = true;

    renderer.render( scene, mirrorCamera );

   // flag for lens flares to switch off/on visibility to avoid reflections
   // on to-do list to fix
    mirrorCamRendering = false;

    if (renderer.render( scene, mirrorCamera ) == true)
    {
        mirrorCamRendering = true;
    }
    else
...
//  PUZZLE CORE CODE
            ...
            function makeOcean()
            {
                ...
                function init() {
                    ...
                    const waterGeometry = new THREE.PlaneBufferGeometry( oceanWidth, oceanDepth );

                    const textureLoader = new THREE.TextureLoader();
                    const loadedTexture = textureLoader.load( waternormalsJPG );
                    loadedTexture.wrapS = THREE.RepeatWrapping;
                    loadedTexture.wrapT = THREE.RepeatWrapping;
                    // console.log('loadedTexture Ocean waternormalsJPG: ', loadedTexture);
                    ...
                    // console.log('sunFromSkyCheckbox:', sunFromSkyCheckbox);
                    // if ( sunFromSkyCheckbox == true )
                    // {
                    //     // set sun to skySun
                    // };
                    ...
                    // waterUniforms.size = textureSize;
                    // waterUniforms.size = 10;
                    ...
                    // 	sun.setFromSphericalCoords( 1, phi, theta );
                        ...
                        // sky.material.uniforms[ 'sunPosition' ].value.copy( skySunVector );
                        water.material.uniforms[ 'sunDirection' ].value.copy( skySunVector ).normalize();

                        scene.environment = pmremGenerator.fromScene( skyWin ).texture;
...
// Describe this function...
function GENERATE_SKY_AND_OCEAN() {
    ...
    console.log(getObjTransform('skySunDirLight', 'position', 'x'));
    console.log(getObjTransform('skySunDirLight', 'position', 'y'));
    console.log(getObjTransform('skySunDirLight', 'position', 'z'));
  });
}
...
function MAIN() {
  ...
  tweenCamera('', 'Lens_Flare_Point_Light', 0.5, function() {}, 1);
