...
function make_CSS_LISTS()
{
  for( let i = 1; i < ( LIST_pluginPack_plugins.length ); i++ )
  {
...
function make_site_HTML()
{
  ...
  makeALLPageImages ( '_plugin', 'width: 75%');
  makeALLPageImages ( '_example', 'width: 85%');
  makeALLPageImages ( '_console', 'width: 35%');
...
function makeALLPageImages(suffix, width)
{
  ...
  for( let i = 0; i < ( LIST_pluginPack_plugins.length + 1 ); i++ )
  {
    ...
    makeSinglePageImage ( item, suffix, width )
...
function makeSinglePageImage ( imageName, suffix, width )
{
  ...
  function checkIfImageExistsCallback(boolean)
  {
    ...
    else if ( boolean == true )
    {
      ...
      // for( let i = 1; i < ( LIST_pluginPack_plugins.length + 2 ); i++ )
      for( let i = 1; i < ( LIST_pluginPack_plugins.length + 1 ); i++ )
      {

        let pageImage = document.createElement('img');   
        pageImage.src = imageNameFull;

        console.log('imgCounter (i):',i);
